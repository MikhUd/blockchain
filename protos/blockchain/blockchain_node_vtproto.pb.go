...
func (m *Packet) CloneVT() *Packet {
	...
	if rhs := m.Targets; rhs != nil {
		tmpContainer := make([]*node.PID, len(rhs))
		for k, v := range rhs {
			if vtpb, ok := interface{}(v).(interface{ CloneVT() *node.PID }); ok {
				tmpContainer[k] = vtpb.CloneVT()
			} else {
				tmpContainer[k] = proto.Clone(v).(*node.PID)
			}
		}
...
func (m *TransactionRequest) CloneVT() *TransactionRequest {
	...
	r := &TransactionRequest{
		...
		SenderPrivateKey:           m.SenderPrivateKey,
...
func (this *Packet) EqualVT(that *Packet) bool {
	...
	for i, vx := range this.Targets {
		...
		if p, q := vx, vy; p != q {
			...
			if equal, ok := interface{}(p).(interface{ EqualVT(*node.PID) bool }); ok {
				if !equal.EqualVT(q) {
					return false
				}
...
func (this *TransactionRequest) EqualVT(that *TransactionRequest) bool {
	...
	if this.SenderPrivateKey != that.SenderPrivateKey {
		return false
	}
...
// BlockchainNodeClient is the client API for BlockchainNode service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type BlockchainNodeClient interface {
	Receive(ctx context.Context, opts ...grpc.CallOption) (BlockchainNode_ReceiveClient, error)
}

type blockchainNodeClient struct {
	cc grpc.ClientConnInterface
}

func NewBlockchainNodeClient(cc grpc.ClientConnInterface) BlockchainNodeClient {
	return &blockchainNodeClient{cc}
}

func (c *blockchainNodeClient) Receive(ctx context.Context, opts ...grpc.CallOption) (BlockchainNode_ReceiveClient, error) {
	stream, err := c.cc.NewStream(ctx, &BlockchainNode_ServiceDesc.Streams[0], "/blockchain.BlockchainNode/Receive", opts...)
	if err != nil {
		return nil, err
	}
	x := &blockchainNodeReceiveClient{stream}
	return x, nil
}

type BlockchainNode_ReceiveClient interface {
	Send(*Packet) error
	Recv() (*Packet, error)
	grpc.ClientStream
}

type blockchainNodeReceiveClient struct {
	grpc.ClientStream
}

func (x *blockchainNodeReceiveClient) Send(m *Packet) error {
	return x.ClientStream.SendMsg(m)
}

func (x *blockchainNodeReceiveClient) Recv() (*Packet, error) {
	m := new(Packet)
...
// BlockchainNodeServer is the server API for BlockchainNode service.
// All implementations must embed UnimplementedBlockchainNodeServer
// for forward compatibility
type BlockchainNodeServer interface {
	Receive(BlockchainNode_ReceiveServer) error
	mustEmbedUnimplementedBlockchainNodeServer()
}

// UnimplementedBlockchainNodeServer must be embedded to have forward compatible implementations.
type UnimplementedBlockchainNodeServer struct {
}

func (UnimplementedBlockchainNodeServer) Receive(BlockchainNode_ReceiveServer) error {
	return status.Errorf(codes.Unimplemented, "method Receive not implemented")
}
func (UnimplementedBlockchainNodeServer) mustEmbedUnimplementedBlockchainNodeServer() {}

// UnsafeBlockchainNodeServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to BlockchainNodeServer will
// result in compilation errors.
type UnsafeBlockchainNodeServer interface {
	mustEmbedUnimplementedBlockchainNodeServer()
}

func RegisterBlockchainNodeServer(s grpc.ServiceRegistrar, srv BlockchainNodeServer) {
	s.RegisterService(&BlockchainNode_ServiceDesc, srv)
}

func _BlockchainNode_Receive_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(BlockchainNodeServer).Receive(&blockchainNodeReceiveServer{stream})
}

type BlockchainNode_ReceiveServer interface {
	Send(*Packet) error
	Recv() (*Packet, error)
	grpc.ServerStream
}

type blockchainNodeReceiveServer struct {
	grpc.ServerStream
}

func (x *blockchainNodeReceiveServer) Send(m *Packet) error {
	return x.ServerStream.SendMsg(m)
}

func (x *blockchainNodeReceiveServer) Recv() (*Packet, error) {
	m := new(Packet)
...
// BlockchainNode_ServiceDesc is the grpc.ServiceDesc for BlockchainNode service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var BlockchainNode_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "blockchain.BlockchainNode",
	HandlerType: (*BlockchainNodeServer)(nil),
	Methods:     []grpc.MethodDesc{},
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "Receive",
			Handler:       _BlockchainNode_Receive_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
	Metadata: "blockchain_node.proto",
}
...
func (m *TransactionRequest) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.SenderPrivateKey) > 0 {
		i -= len(m.SenderPrivateKey)
		copy(dAtA[i:], m.SenderPrivateKey)
		i = encodeVarint(dAtA, i, uint64(len(m.SenderPrivateKey)))
		i--
		dAtA[i] = 0x12
	}
...
func (m *Packet) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			m.Targets = append(m.Targets, &node.PID{})
		...
		case 2:
			...
			m.Senders = append(m.Senders, &node.PID{})
...
func (m *TransactionRequest) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SenderPrivateKey", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.SenderPrivateKey = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 3:
			if wireType != 2 {
